#!/bin/bash
 ...
 UNMARK(){ $e "\e[27m";}
  ...
  ARROW(){ IFS= read -s -n1 key 2>/dev/null >&2
     ...
     M0(){ TPUT  4 20; $e "1)Idena Manager";}
     M1(){ TPUT  5 20; $e "2)Shared node";}
...
REFRESH(){ after=$((i+1)); before=$((i-1))
  ...
  while [[ "$O" != " " ]]; do case $i in
        0) S=M0;SC;if [[ $cur == enter ]];then R;reset && bash /home/idena-aio/idena-manager/idena-manager-menu.sh ;fi;;
        1) S=M1;SC;if [[ $cur == enter ]];then R;reset && bash /home/idena-aio/shared-node/shared-node-menu.sh ;fi;;
